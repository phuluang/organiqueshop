<?php
	class ModelSimpleBlogAuthor extends Model {
		...
		public function addAuthor($data) {
			...
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'simple_blog_author_id=" . (int)$simple_blog_author_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			}
		...
		public function editAuthor($simple_blog_author_id, $data) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'simple_blog_author_id=" . (int)$simple_blog_author_id . "'");
		...
		public function deleteAuthor($simple_blog_manager_id) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'simple_blog_author_id=" . (int)$simple_blog_manager_id . "'");
	
			$this->cache->delete('blog_author');
		}
		
		public function getAuthor($simple_blog_author_id) {
			$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'simple_blog_author_id=" . (int)$simple_blog_author_id . "') AS keyword FROM `" . DB_PREFIX . "simple_blog_author` WHERE simple_blog_author_id = '" . (int)$simple_blog_author_id . "'");	
			return $query->row;
		}		
